...
Com `npx live-server` rodando, use o Playwright (já instalado) ou o MCP do Chrome DevTools para checar nos tamanhos 360×640, 390×844, 414×896 e 768×1024:
...
Sem `--host=0.0.0.0` o `live-server` escuta só em `localhost` e o celular
não enxerga.
...
O Firewall do Windows costuma bloquear a porta 8080 (padrão do
`live-server`) para outros dispositivos. Peça ao usuário para rodar num
**PowerShell como administrador** (não execute por ele — exige elevação):

```powershell
New-NetFirewallRule -DisplayName "live-server 8080" -Direction Inbound -LocalPort 8080 -Protocol TCP -Action Allow -Profile Private
```

> Regra restrita ao perfil **Private**. Não aplique em rede pública, e remova depois com `Remove-NetFirewallRule -DisplayName "live-server 8080"` se for um ambiente compartilhado.

### 2.4 Abrir no celular

Instrua: conectar o celular **na mesma rede Wi-Fi** e abrir `http://SEU_IP:8080`.
...
4. o `live-server` subiu com `--host=0.0.0.0`?
...
### 2.6 Não há "build de produção" separado

Este é um site estático sem etapa de build: os arquivos servidos pelo
`live-server` em dev são **os mesmos** que vão para o GitHub Pages — não
existe uma otimização adicional (minificação, compressão de imagem) que só
aparece em produção, a menos que o projeto adote isso manualmente no
futuro. Uma única passagem do roteiro (Etapa 3) já reflete o que vai ao
ar; não é necessário repetir em "modo produção".
