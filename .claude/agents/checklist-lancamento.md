...
1. **`paginas-de-estado`** (skill) — `404.html` do GitHub Pages e tratamento de erro em JS no cliente. Base para as demais: se a página quebrar visualmente aqui, as etapas seguintes herdam o problema.
...
6. **`auditor-segredos`** (agent, via Agent tool) — roda **depois** das anteriores de propósito: elas introduzem constantes novas no JS (GA ID, DSN do Sentry) e é isso que a auditoria de segredos precisa conferir — nenhuma delas é secreta por natureza, mas vale confirmar que nenhum valor realmente sensível foi hardcoded junto.
...
Para os passos 1–5, invoque a skill correspondente com a ferramenta `Skill` (`{"skill": "nome-da-skill"}`) e siga as instruções carregadas — inclusive os "Passo 0" de cada uma, que perguntam dado que só o usuário sabe (domínio, GA ID, DSN, contato de alerta, template de política de privacidade). Este projeto não tem type-checker nem bundler — depois de cada skill, rode a checagem real do projeto:
...
Se falhar, corrija antes de seguir para a próxima etapa — não empilhe erro de lint de uma etapa em cima da outra. Se `.eslintrc`/`eslint.config.js` ou `.stylelintrc.json` ainda não existirem no projeto, diga isso explicitamente no relatório em vez de tratar a etapa como "passou" — um lint que não roda não é um lint limpo.
...
- sem domínio definido → pule a parte de Search Console e a verificação de preview social, mas termine metadata/OG/sitemap/robots usando a URL do GitHub Pages já em uso (`https://douglasfernandesdev.github.io/Portif-lio/`) e documente se ela for só provisória;
...
1. **Feito e verificado** — por etapa, o que foi criado/alterado e o resultado real de `eslint`/`stylelint`/testes rodados (nunca "deveria passar").
2. **Pendente de informação sua** — lista objetiva: o quê, e onde usar assim que responder (ex.: "GA ID → constante `GA_ID` em `js/modules/consentimento-cookies.js`").
